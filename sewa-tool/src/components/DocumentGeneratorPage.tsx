...
export function DocumentGeneratorPage({ config }: { config: DocumentTemplateConfig }) {
  ...
  const yearBundles = useMemo(
    ...
    [config.filenamePrefix, documentData.hca, documentData.participant, yearGroups, yearOrders],
  );
  ...
  const handleGenerate = async (format: "docx" | "pdf") => {
    if (!validation.success || !previewDates.length || !yearBundles.length) return;
    ...
    try {
      for (const bundle of yearBundles) {
        ...
        const outputData = { ...documentData, year: bundle.year };
    ...
    <AppShell activeTool={config.title}>
      ...
      <div className="workflow-grid">
        ...
        <aside className="panel side-panel-card">
          <div className="section-header"><div><p className="section-number">04</p><h3>PREVIEW</h3></div></div>
          <div className="summary-box"><p>Participant</p><strong>{documentData.participant || "-"}</strong><p>HCA</p><strong>{documentData.hca || "-"}</strong><p>Supervisor</p><strong>{documentData.supervisor || "-"}</strong><p>DATES TO ADD</p><strong>{previewDates.length}</strong><p>Year(s)</p><strong>{yearOrders.length ? yearOrders.join(", ") : "-"}</strong><p>File names</p>{yearBundles.length ? yearBundles.map((bundle) => <strong key={`${bundle.year}-files`} className="mono">{bundle.files.docx}<br />{bundle.files.pdf}</strong>) : <strong className="mono">No files ready yet</strong>}</div>
          <div className="action-stack">
            <Button variant="primary" disabled={!validation.success || !previewDates.length || isGenerating} onClick={() => void handleGenerate("docx")}>{isGenerating ? "Generating..." : "Generate DOCX"}</Button>
            <Button variant="secondary" disabled={!validation.success || !previewDates.length || isGenerating} onClick={() => void handleGenerate("pdf")}>Generate PDF</Button>
          </div>
        </aside>
      </div>
    </AppShell>
